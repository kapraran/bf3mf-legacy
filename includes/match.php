...
class Match
{
    private $C_presets = array('no' => 'Normal', 'cu' => 'Custom', 'hc' => 'Hardcore', 'in' => 'Infantry');
    private $C_modes = array('cq' => 'Conquest', 'tdm' => 'Team Deathmatch', 'rs' => 'Rush', 'sqrs' => 'Squad Rush');
    private $C_platforms = array('pc' => 'PC', 'ps3' => 'Playstation 3', 'x360' => 'XBOX 360');
    ...
    public function prep_match_list($m1, $m2, $m3)
    {
        ...
        $maps = array();
        if ($m1 !== '') {
            $maps[$m1] = array('name' => $MAP_LIST[$m1]['name'], 'image' => $MAP_LIST[$m1]['img_wide'], 'icon' => $MAP_LIST[$m1]['icon']);
        }

        if ($m2 !== '') {
            $maps[$m2] = array('name' => $MAP_LIST[$m2]['name'], 'image' => $MAP_LIST[$m2]['img_wide'], 'icon' => $MAP_LIST[$m2]['icon']);
        }

        if ($m3 !== '') {
            $maps[$m3] = array('name' => $MAP_LIST[$m3]['name'], 'image' => $MAP_LIST[$m3]['img_wide'], 'icon' => $MAP_LIST[$m3]['icon']);
        }
    ...
    public function generate_match_opponent($mid)
    {
        ...
        $q = $MYSQL->query("SELECT from_clan_id FROM challenges WHERE match_id = {$mid} AND accepted = 1 LIMIT 0,1");
        if (!$q) {
            redirectHTML('error/1');
        ...
        if ($clan_info !== false) {
            ...
            $HTML .= '<div class="match-clan-card">
                                <div class="avatar">
                                    <img src="' . $clan_info['avatar'] . '" alt="no logo" />
                                </div>
                                <div class="match-clan-info">
                                    <div class="name">' . $clan_info['name'] . '</div>
                                    <div class="b-i"><span class="tag">[' . $clan_info['tag'] . ']</span> <span class="country"> <img src="img/flags/' . $clan_info['country'] . '.png" alt="' . $COUNTRIES[$clan_info['country']] . '"> ' . $COUNTRIES[$clan_info['country']] . '</span>';
            if ($LOGGED) {
                $HTML .= generate_info_btn($clan_info['id'], false) . generate_send_msg_btn($clan_info['id'], false);
            }
    ...
    public function already_challenged($mid)
    {
        ...
        $r = $MYSQL->query(" SELECT * FROM challenges WHERE match_id = {$mid} AND from_clan_id = {$_SESSION['id']} ");
        if (!$r) {
            redirectHTML('error/1');
        } else {
    ...
    public function active_challenges()
    {
        ...
        $r = $MYSQL->query(" SELECT * FROM challenges WHERE from_clan_id = {$_SESSION['id']} ");
        if (!$r) {
            redirectHTML('error/1');
        } else {
    ...
    public function check_if_accepted($mid)
    {
        ...
        $r = $MYSQL->query("SELECT * FROM challenges WHERE match_id = {$mid} AND accepted = 1");
        if (!$r) {
            redirectHTML('error/1');
        } else {
            return (mysql_num_rows($r) > 0) ? true : false;
    ...
    public function get_challenges($mid)
    {
        ...
        $array = array();
        $r = $MYSQL->query("SELECT * FROM challenges WHERE match_id = {$mid} AND rejected = 0");
        if (!$r) {
            redirectHTML('error/1');
        } else {
    ...
    public function clear_challenges($mid)
    {
        ...
        $q = $MYSQL->query("SELECT from_clan_id FROM challenges WHERE match_id = {$mid}");
        if (!$q) {
            redirectHTML('error/1');
        }

        while ($notif_ar = mysql_fetch_array($q)) {
            add_notification("<b>Match #{$mid}</b> deleted by <b>{$_SESSION['name']}</b>. Your challenge for <b>match #{$mid}</b> does no longer exist", $_SESSION['id'], $notif_ar['from_clan_id']);
        }
    ...
    public function reject_other_challenges_than($cid, $matchid)
    {
        ...
        $q = $MYSQL->query("SELECT from_clan_id FROM challenges WHERE id <> {$cid} AND match_id = {$matchid} AND rejected = 0");
        if (!$q) {
            redirectHTML('error/1');
        }

        // reject challenges
        $r = $MYSQL->query("UPDATE challenges SET rejected = 1 WHERE id <> {$cid} AND match_id = {$matchid} AND rejected = 0");
        if (!$r) {
            redirectHTML('error/1');
        }
