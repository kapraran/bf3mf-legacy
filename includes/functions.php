...
{
    $ret = (isset($_POST[$par]) && $_POST[$par] !== '') ? addslashes($_POST[$par]) : false;
    if ($ret !== false) {
        return $ret;
    } else {
        return $def;
...
{
    $ret = (isset($_GET[$par]) && $_GET[$par] !== '') ? addslashes($_GET[$par]) : false;
    if ($ret !== false) {
        return $ret;
    } else {
        return $def;
...
function EmailValidation($email)
{
    ...
    if (preg_match('#[a-z||0-9]\@[a-z||0-9].[a-z]#i', $email)) { //checks to make sure the email address is in a valid format
...
function generate_info_btn($id, $show = true)
{
    ...
    if ($show) {
        echo '<span class="clan-profile-popup" data-plugin="jpopup" data-height="400" data-width="510" data-type="ajax" data-typevalue="ajax.php?mode=clan&cid=' . $id . '&secid=' . $_SESSION['secid'] . '">info</span>';
    } else {
        return '<span class="clan-profile-popup" data-plugin="jpopup" data-height="400" data-width="510" data-type="ajax" data-typevalue="ajax.php?mode=clan&cid=' . $id . '&secid=' . $_SESSION['secid'] . '">info</span>';
    }
...
function generate_send_msg_btn($id, $show = true)
{
    ...
    if ($show) {
        echo '<span class="clan-profile-popup" data-plugin="jpopup" data-height="310" data-width="470" data-type="ajax" data-typevalue="ajax.php?mode=msg&cid=' . $id . '&secid=' . $_SESSION['secid'] . '">send message</span>';
    } else {
        return '<span class="clan-profile-popup" data-plugin="jpopup" data-height="310" data-width="470" data-type="ajax" data-typevalue="ajax.php?mode=msg&cid=' . $id . '&secid=' . $_SESSION['secid'] . '">send message</span>';
    }
...
function random_string()
{
    ...
    $character_set_array[] = array('count' => 12, 'characters' => 'abcdefghijklmnopqrstuvwxyz');
...
{
    global $MYSQL;
    $r = $MYSQL->query("SELECT * FROM notifications WHERE to_id = {$_SESSION['id']} AND opened = 0");
    if (!$r) {
        redirectHTML('error/1');
    } else {
...
{
    global $MYSQL;
    $r = $MYSQL->query("UPDATE notifications SET opened = 1 WHERE id <= {$maxid} AND id >= {$minid} AND opened = 0 AND to_id = {$_SESSION['id']}");
    if (!$r) {
        redirectHTML('error/1');
    } else {
        return true;
