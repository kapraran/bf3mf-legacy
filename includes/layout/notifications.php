...
if ($rows > 0) {
    ...
    // new query
    $query .= " ORDER BY time DESC LIMIT " . (($CURRENT_PAGE - 1) * $NOTIFICATIONS_PER_PAGE) . "," . $NOTIFICATIONS_PER_PAGE;
    ...
    <?php if ($rows > 0): ?>
            ...
            <table id="table-notifs">
                ...
                <tbody>
                    ...
                    <?php while ($notifs = mysql_fetch_array($display_q)): ?>

                    <tr class="notif<?php echo ($notifs['opened'] == 0) ? ' unread' : ''; ?>"  >
                        <td class="read"><?php echo ($notifs['opened'] == 0) ? '<div class="unread"></div>' : ''; ?></td>
                        <td class="message"><?php echo htmlspecialchars_decode($notifs['content']); ?></td>
                        <td class="by"><?php echo $notifs['from_name']; ?></td>
                        <td class="time"><?php echo date('j F Y, H:i', $notifs['time']); ?></td>
                    </tr>
