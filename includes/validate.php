...
function valid_sregister($pars)
{
    ...
    if (strlen($pars['password']) < 6 || strlen($pars['password']) > 20 || !checklatin($pars['password'])) {
    ...
    if (strlen($pars['clanname']) > 100 || strlen($pars['clanname']) < 5 || !checklatin($pars['clanname'], true)) {
...
function validate_create_match($pars)
{
    ...
    numeric(array($pars['start_day'], $pars['start_month'], $pars['start_year'], $pars['start_hour'], $pars['start_min']), 'none');

    // server region
    if (!array_search($pars['server_region'], array('ZERO_INDEX', 'US', 'EU', 'AS'))) {
        redirectHTML('create/error/1');
    }

    // start day
    if ($pars['start_day'] == 'none' || $pars['start_day'] > 31 || $pars['start_day'] < 1) {
        redirectHTML('create/error/2');
    }

    // start month
    if ($pars['start_month'] == 'none' || $pars['start_month'] > 12 || $pars['start_month'] < 1) {
        redirectHTML('create/error/3');
    }

    // start year
    if ($pars['start_year'] == 'none' || $pars['start_year'] > 2013 || $pars['start_year'] < 2012) {
        redirectHTML('create/error/4');
    }

    // start hour
    if ($pars['start_hour'] == 'none' || $pars['start_hour'] > 23 || $pars['start_hour'] < 0) {
        redirectHTML('create/error/5');
    }

    // start min
    if ($pars['start_min'] == 'none' || $pars['start_min'] > 59 || $pars['start_min'] < 0) {
    ...
    // time after
    if ($pars['time_after'] == 'none' || $pars['time_after'] > 12 || $pars['time_after'] < 2) {
    ...
    // preset
    if (!array_search($pars['preset'], array('ZERO_INDEX', 'no', 'hc', 'in', 'cu'))) {
        redirectHTML('create/error/10');
    }

    // mode
    if (!array_search($pars['mode'], array('ZERO_INDEX', 'cq', 'rs', 'sqrs', 'tdm'))) {
        redirectHTML('create/error/11');
    }

    // platform
    if (!array_search($pars['platform'], array('ZERO_INDEX', 'pc', 'ps3', 'x360'))) {
        redirectHTML('create/error/12');
    }

    // team size
    if (!array_search($pars['tsize'], array('ZERO_INDEX', 4, 8, 12, 16, 24, 32))) {
...
function validate_update_settings($pars)
{
    ...
    // clan logo
    if (isset($pars['clanlogo']) && strlen(trim($pars['clanlogo'])) > 0 && !isValidURL($pars['clanlogo'])) {
        redirectHTML('controlpanel/error/2');
    }

    // battlelog profile url
    $b_log_url = (strlen(trim($pars['battlelog'])) > 0) ? parse_url($pars['battlelog']) : array('host' => 'error');
    if (strlen(trim($pars['battlelog'])) > 0 && (!isValidURL($pars['battlelog']) || $b_log_url['host'] !== 'battlelog.battlefield.com')) {
...
function validate_find_match($pars)
{
    ...
    // check if not numeric values
    numeric(array($pars['start_day'], $pars['start_month'], $pars['start_year'], $pars['start_hour'], $pars['start_min'], $pars['end_day'], $pars['end_month'], $pars['end_year'], $pars['end_hour'], $pars['end_min']), 'none');

    // server region
    if ($pars['server_region'] != 'all' && !array_search($pars['server_region'], array('ZERO_INDEX', 'US', 'EU', 'AS'))) {
    ...
    // end day
    if ($pars['end_day'] == 'none' || $pars['end_day'] > 31 || $pars['end_day'] < 1) {
        redirectHTML('find/error/3');
    }

    // end month
    if ($pars['end_month'] == 'none' || $pars['end_month'] > 12 || $pars['end_month'] < 1) {
        redirectHTML('find/error/3');
    }

    // end year
    if ($pars['end_year'] == 'none' || $pars['end_year'] > 2013 || $pars['end_year'] < 2012) {
        redirectHTML('find/error/3');
    }

    // end hour
    if ($pars['end_hour'] == 'none' || $pars['end_hour'] > 23 || $pars['end_hour'] < 0) {
        redirectHTML('find/error/3');
    }

    // end min
    if ($pars['end_min'] == 'none' || $pars['end_min'] > 59 || $pars['end_min'] < 0) {
        redirectHTML('find/error/3');
    }
    ...
    // preset
    if ($pars['preset'] != 'all' && !array_search($pars['preset'], array('ZERO_INDEX', 'no', 'hc', 'in', 'cu'))) {
        redirectHTML('find/error/4');
    }

    // mode
    if ($pars['mode'] != 'all' && !array_search($pars['mode'], array('ZERO_INDEX', 'cq', 'rs', 'sqrs', 'tdm'))) {
        redirectHTML('find/error/5');
    }

    // platform
    if ($pars['platform'] != 'all' && !array_search($pars['platform'], array('ZERO_INDEX', 'pc', 'ps3', 'x360'))) {
        redirectHTML('find/error/6');
    }

    // team size
    if ($pars['tsize'] != 'all' && !array_search($pars['tsize'], array('ZERO_INDEX', 4, 8, 12, 16, 24, 32))) {
