...
$clan_id_res = $MATCH->reject_other_challenges_than($MODE_PARS['cid'], $MODE_PARS['matchid']);

// send a notification to the creator of the challenge (rejected)
$notification_content = '<b>' . $_SESSION['name'] . '</b> rejected your challenge for <a href="./match/' . $MODE_PARS['matchid'] . '">match #' . $MODE_PARS['matchid'] . '</a>';
while ($clan_id_array = mysql_fetch_array($clan_id_res)) {
    add_notification($notification_content, $_SESSION['id'], $clan_id_array['from_clan_id']);
}

// accept the challenge
$q = $MYSQL->query("UPDATE challenges SET accepted = 1 WHERE id = {$challenge_info['id']}");
if (!$q) {
    redirectHTML('error/1');
}

// send a notification to the creator of the challenge (accepted)
$notification_content = '<b>' . $_SESSION['name'] . '</b> accepted your challenge for <a href="./match/' . $MODE_PARS['matchid'] . '">match #' . $MODE_PARS['matchid'] . '</a>. Contact the leader of this clan to arrange the details for this match!';
add_notification($notification_content, $_SESSION['id'], $challenge_info['from_clan_id']);
